...
public class AVFrame {
	...
	public AVFrame(long frameNo, byte frameState,byte[] frameData, int frameDataSize) {

		codec_id = MEDIA_CODEC_VIDEO_H264;//Packet.byteArrayToShort_Little(frameHead, 0);
		frmState = frameState;
		flags = IPC_FRAME_FLAG_IFRAME;//frameHead[2];
		// camIndex = frameHead[3];
		onlineNum =1; //frameHead[4];
		timestamp =1; //Packet.byteArrayToInt_Little(frameHead, 12);
		videoWidth =320; //frameHead.length > 16 ? Packet.byteArrayToInt_Little(frameHead, 16) : 0;
		videoHeight = 480;//frameHead.length > 16 ? Packet.byteArrayToInt_Little(frameHead, 20) : 0;
		frmSize = frameDataSize;
		frmData = frameData;
		frmNo = frameNo;
	}
